...
--      address, contact number, email, emergency contact) become nullable —
--      display-only fields, safe to leave blank until HR fills them in.
--   2. Employment status and payroll type get a new 'unassigned' enum value,
--      and a real "Unassigned" department + position are added — used only
--      for historical/resigned employees imported without this on file.
--      (department_id/position_id stay NOT NULL since scoping/reports
--      throughout the app assume every employee has a real one.)
--
