          ...
          <div className="relative">
            ...
            {userMenu && (
              <div className="absolute right-0 mt-2 w-64 rounded-xl border border-[var(--border-hairline)] bg-[var(--surface-1)] p-2 shadow-lg">
                ...
                >
                  <LogOut size={16} /> {isRealAccount ? "Sign out" : "Switch demo user"}
                </button>
              </div>
            )}
          </div>
      ...
      <Modal open={changingPassword} onClose={() => setChangingPassword(false)} title="Change password">
        {passwordChanged ? (
          <div className="space-y-3">
            <div className="text-sm text-[var(--text-secondary)]">Your password has been changed. Use the new one next time you sign in.</div>
            <div className="flex justify-end">
              <button onClick={() => setChangingPassword(false)} className="rounded-lg bg-[var(--series-1)] px-3 py-1.5 text-sm font-medium text-[var(--on-accent)]">Done</button>
            </div>
          </div>
        ) : (
          <PasswordForm requireCurrent onDone={() => setPasswordChanged(true)} onCancel={() => setChangingPassword(false)} />
        )}
      </Modal>
    </>
  );
