...
import { countUnreadHrMessages, fetchHrInboxMessages, fetchHrThread, HR_MESSAGE_MAX, markHrThreadRead, sendHrMessage, type HrMessage } from "@/lib/supabase/hr-messages";
...
export function ChatThread({ employeeId, viewerIsHr, otherName, onSent }: { employeeId: string; viewerIsHr: boolean; otherName: string; onSent?: () => void }) {
    ...
    fetchHrThread(employeeId).then(
      (m) => {
        ...
        if (m.some((x) => !x.readAt && x.fromHr !== viewerIsHr)) {
          markHrThreadRead(employeeId).then(() => window.dispatchEvent(new Event(READ_EVENT)), () => {});
        }
      },
  ...
  async function send() {
    ...
    setSending(true);
    try {
      const m = await sendHrMessage(employeeId, body);
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
          ...
          return (
            <div key={m.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[85%] sm:max-w-[70%]`}>
                <div
                  className={`rounded-2xl px-3 py-2 text-sm whitespace-pre-wrap [overflow-wrap:anywhere] ${
                    mine ? "rounded-br-sm bg-[var(--series-1)] text-[var(--on-accent)]" : "rounded-bl-sm border border-[var(--border-hairline)] bg-[var(--surface-1)] text-[var(--text-primary)]"
                  }`}
                >
                  {m.body}
                </div>
      ...
      <form
        ...
        className="flex items-end gap-2 border-t border-[var(--border-hairline)] p-2 sm:p-3"
      >
        <textarea
          value={draft}
        ...
        />
        <button type="submit" disabled={!draft.trim() || sending} className="flex h-10 items-center gap-1.5 rounded-lg bg-[var(--series-1)] px-3 text-sm font-medium text-[var(--on-accent)] disabled:opacity-40" aria-label="Send">
...
export function HrInbox() {
  ...
  const conversations = useMemo(() => {
    ...
    for (const m of inbox ?? []) {
      ...
      const c = map.get(m.employeeId) ?? { employeeId: m.employeeId, last: m, unread: 0 };
  ...
  const newMatches = q
    ? employees
        .filter((e) => e.id !== myId && e.status !== "resigned" && e.status !== "terminated" && !conversations.some((c) => c.employeeId === e.id) && fullName(e).toLowerCase().includes(q))
        .slice(0, 8)
    : [];
      ...
      <div className={`w-full shrink-0 flex-col border-r border-[var(--border-hairline)] lg:flex lg:w-80 ${selected ? "hidden" : "flex"}`}>
        <div className="border-b border-[var(--border-hairline)] p-2">
          <label className="flex items-center gap-2 rounded-lg border border-[var(--border-hairline)] px-2.5 py-1.5">
            ...
            <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search or start a new chat" className="w-full bg-transparent text-sm outline-none" aria-label="Search employees" />
          </label>
        </div>
        <div className="min-h-0 flex-1 overflow-y-auto">
          ...
          {shown.map((c) => (
            <button key={c.employeeId} onClick={() => open(c.employeeId)} className={`flex w-full items-start gap-2 border-b border-[var(--gridline)] px-3 py-2.5 text-left hover:bg-[var(--gridline)]/40 ${selected === c.employeeId ? "bg-[var(--series-1)]/10" : ""}`}>
              <div className="min-w-0 flex-1">
                ...
                <div className="truncate text-xs text-[var(--text-secondary)]">{c.last.fromHr ? "You: " : ""}{c.last.body}</div>
              </div>
